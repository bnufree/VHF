...
const QString myAppName = "VHF.exe";
...
VHFMonitorThread::~VHFMonitorThread()
{
    qDebug()<<__FUNCTION__;
    if(mAppID > 0)
    {
...
void VHFMonitorThread::run()
{
    while (1)
    {
        ...
        } else
        {
            ...
            if(fileInfo.lastModified().secsTo(now) >= 20)
            {
...
VHFMonitorThread::VHFMonitorThread(const QString& app, QObject *parent)
    : QThread(parent)
    , mAppName(app)
    , mAppID(-1)
    , mSub(0)
{
    ...
    qDebug()<<"app name:"<<mAppName<<app;
    mDogFile = "watchdog.txt";
